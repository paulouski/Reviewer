...
function generateConfigTabContent(role, roleName) {
    return `
        <div id="tab-${role}" class="config-tab-content hidden">
            <h4 class="text-base sm:text-lg font-semibold mb-3 sm:mb-4">${roleName}</h4>
            <div class="space-y-3 sm:space-y-4">
                <div>
                    ...
                    <select id="config-${role}-model" class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg text-sm sm:text-base"></select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Max Output Tokens</label>
                    <input type="number" id="config-${role}-max-tokens" class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg text-sm sm:text-base" min="50" max="5000">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Reasoning Effort</label>
                    <select id="config-${role}-reasoning" class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg text-sm sm:text-base">
                ...
                <div>
                    ...
                    <textarea id="config-${role}-prompt" rows="8" class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg font-mono text-xs sm:text-sm"></textarea>
                </div>
                <div>
                    ...
                    <textarea id="config-${role}-schema" readonly 
                        class="w-full px-3 sm:px-4 py-2 border border-gray-200 rounded-lg font-mono text-xs sm:text-sm bg-gray-50 text-gray-600" 
                        rows="6"></textarea>
                </div>
            </div>
        </div>
...
function initializeConfigTabs() {
    ...
    const tabContentContainer = configModal.querySelector('.flex-1.overflow-y-auto');
